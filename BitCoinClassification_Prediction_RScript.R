...
bc <- read.csv("BTCFirstData.csv", header = TRUE)
summary(bc)
str(bc)
#bc$PriceDirection <- as.integer(bc$PriceDirection)
...
hist(bc$HashRate)
hist(bc$Difficulty)
hist(bc$CostPerTransaction)
hist(bc$NoOfTransactions)
hist(bc$RevenueUSD)

# Correlation Test
cor(subset(bc, select=c(2:7,11:15)))

# Scaling of numerical variables (values will be between 0 and 1) also called as normalization.
##bc$PriceDirection <- (bc$PriceDirection- min(bc$PriceDirection))/(max(bc$PriceDirection)- min(bc$PriceDirection))
bc$Open.<- (bc$Open.- min(bc$Open.))/(max(bc$Open.)- min(bc$Open.))
bc$High<-(bc$High- min(bc$High))/(max(bc$High)- min(bc$High))
bc$Low <-(bc$Low - min(bc$Low))/ (max(bc$Low)- min(bc$Low))
bc$Close..<-(bc$Close..- min(bc$Close..))/(max(bc$Close..)- min(bc$Close..))
bc$Volume<-(bc$Volume- min(bc$Volume))/(max(bc$Volume)- min(bc$Volume))
bc$Market.Cap<-(bc$Market.Cap- min(bc$Market.Cap))/(max(bc$Market.Cap)- min(bc$Market.Cap))
bc$HashRate <- (bc$HashRate- min(bc$HashRate))/(max(bc$HashRate)- min(bc$HashRate))
bc$Difficulty <- (bc$Difficulty- min(bc$Difficulty))/(max(bc$Difficulty)- min(bc$Difficulty))
bc$CostPerTransaction <- (bc$CostPerTransaction- min(bc$CostPerTransaction))/(max(bc$CostPerTransaction)- min(bc$CostPerTransaction))
bc$NoOfTransactions <- (bc$NoOfTransactions- min(bc$NoOfTransactions))/(max(bc$NoOfTransactions)- min(bc$NoOfTransactions))
bc$RevenueUSD <- (bc$RevenueUSD- min(bc$RevenueUSD))/(max(bc$RevenueUSD)- min(bc$RevenueUSD))


working_colomns <- bc[c(2:7,11:16)]
str(working_colomns)
# partioning the data train and test
part <- sample(2,nrow(working_colomns), replace = TRUE, prob =c(.50,0.50))
trainingset <- working_colomns[part==1,]
testingset <- working_colomns[part==2,]
...
library(neuralnet)
set.seed(321)
n= neuralnet(PriceDirection~Open.+High+Low+Close..+Volume+Market.Cap+HashRate+Difficulty+CostPerTransaction+NoOfTransactions+RevenueUSD,
             data= trainingset,
             hidden =c(2,1),
             err.fct ="ce",
             linear.output = FALSE,
...
#Prediction
str(trainingset)
predict_result<-neuralnet::compute(n,trainingset[ ,1:11])
predict_result
head(predict_result$net.result)
head(trainingset[1,])

#Confusion Matrix and Classification Error of training data
predict_result<-neuralnet:: compute(n,trainingset[,-12])
p1 <- predict_result$net.result
predict1 <- ifelse(p1>0.5, 1, 0)
confusiontable <- table(predict1,trainingset$PriceDirection)
confusiontable
accuracy <-sum(diag(confusiontable))/sum(confusiontable)
misclassification <- 1-accuracy 
#Print values of accuracy and misclassification
accuracy
misclassification

#Confusion Matrix and Classification Error of testing data
predict_result<-neuralnet:: compute(n,testingset[,-12])
p2 <- predict_result$net.result
predict2 <- ifelse(p2>0.5, 1, 0)
confusiontable <- table(predict2,testingset$PriceDirection)
confusiontable
accuracytest <-sum(diag(confusiontable))/sum(confusiontable)
misclassificationtest <- 1-accuracytest 
#Print values of accuracy and misclassification
accuracytest
...
################### CLASSIFICATION-LSTM
setwd("C:\\NCI Materials\\ADM\\CA2 Project")
finaldata1 <- read.csv("BTCFullData.csv")
...
library(dummies)
X = dummy.data.frame(finaldata1[, -p])
Y = finaldata1[, p]
data = cbind(X, Y)
set.seed(1690)
Ind = sample(N, N*0.8, replace = FALSE) 
p = ncol(data)
xtrain = data.matrix(data[Ind, p])
xtrain  = data.matrix(data[Ind, -p])

ytest = data.matrix(data[-Ind, p])
ytest  = data.matrix(data[-Ind, -p])
...
plottrace = model %>% fit(xtrain, ytrain, epochs = 50, batch_size = 150
                          ,validation_split = 0.3)
#track
#callbacks = callback_early_stopping(patience = 5, monitor = 'acc'), --early stopping - include it in above function
# to plot the graph plot(plottrace)

##prediction 
pred <- model %>% predict(xtest, batch_size = 150)
ypred=pred
ypred = round(pred)

# Confusion matrix
#matrix=metrics.confusion_matrix(ytest.argmax(axis=1),ypred.argmax(axis=1)) 
CM = table(ypred, ytest )
CM
# evaluate the model
evalresult <- model %>% evaluate(xtest, ytest, batch_size = 150)
#Classification
(accuracy = evalresult[2][[1]]* 100)
LossVl = evals[1][[1]]
